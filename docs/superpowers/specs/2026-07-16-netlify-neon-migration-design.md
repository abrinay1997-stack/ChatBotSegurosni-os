...
| `src/persistence/db.ts` | Driver `better-sqlite3` (síncrono) → `@neondatabase/serverless` + `drizzle-orm/neon-http` (async, HTTP — no TCP). Cambia la firma de `db.run/get/all` a `Promise`. Se elimina la creación de directorio local (`mkdirSync`), ya no aplica. Se elige el driver HTTP de Neon (en vez de un driver TCP genérico tipo `postgres-js`) porque está pensado para entornos serverless: cada query es un request HTTP independiente, sin pool de conexiones que gestionar ni riesgo de agotar conexiones entre invocaciones concurrentes de la función. |
...
| `netlify/functions/telegram.mts` (nuevo) | Entry point serverless en el formato moderno de Netlify Functions (v2: `Request`/`Response` web-estándar, confirmado contra la documentación oficial de Netlify vía su MCP). Arma la misma composición de dependencias que `src/index.ts` (extraída a una función compartida `buildBot()` para no duplicar) y expone el resultado de `webhookCallback(bot, "std/http", { secretToken })` — grammY no tiene un adaptador `"netlify"` propio, pero su adaptador `"std/http"` (pensado para `Deno.serve`/web-estándar) tiene exactamente la firma `(req: Request) => Promise<Response>` que Netlify Functions v2 espera. |
