...
| `netlify/functions/telegram.ts` (nuevo) | Entry point serverless (formato clásico de Netlify Functions v1, compatible con AWS Lambda). Arma la misma composición de dependencias que `src/index.ts` (extraída a una función compartida `buildBot()` para no duplicar) y expone `webhookCallback(bot, "aws-lambda-async")` — grammY no tiene un adaptador `"netlify"` propio; Netlify Functions v1 usa el mismo formato de handler `(event, context) => response` que AWS Lambda, así que ese es el adaptador correcto. |
