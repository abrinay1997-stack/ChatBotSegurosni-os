...
## [2026-07-19] Respuestas incoherentes del bot — arquitectura "tool-heavy" con RAG frágil. Rediseño a RAG-first

**Contexto:** el bot daba respuestas incoherentes ("parece que no hay planes disponibles" justo después de listarlos) y a veces caía al fallback genérico.
**Error/síntomas:** (1) reproducido: "¿qué planes hay?" hacía que la LLM llamara la tool `showPlans` dos veces seguidas y Groq devolvía `400 "Failed to call a function"`. (2) "¿qué planes de seguro ofrecen?" → el RAG (`websearch_to_tsquery`, semántica AND) devolvía CERO chunks porque ningún fragmento tenía todos los términos juntos → la LLM sin contexto no podía nombrar los planes. (3) el tool-loop, al agotar rondas con el modelo aún pidiendo tools, devolvía `finalResponse` vacío → "No tengo respuesta".
**Causa raíz:** arquitectura "tool-heavy": para cada mensaje la LLM tenía que llamar herramientas de RECUPERACIÓN (showPlans, lookupKnowledge, getProductInfo, recommendPlan) antes de responder. Eso: (a) sumaba 2-3 llamadas LLM por mensaje (~1400 tokens de esquemas de tools por llamada) quemando la cuota de Groq 2-3x más rápido; (b) hacía RAG por duplicado (el pipeline ya inyectaba contexto + cada tool re-hacía RAG); (c) exponía la fragilidad del tool-calling de Groq (400) y del full-text AND (recall bajo).
**Fix (rediseño RAG-first):** una sola llamada LLM por mensaje, que responde LIBRE y humana anclada en el contexto. Concretamente: prompt v2 cálido con anclaje suave (`v2.system.ts`); se quitan las tools de recuperación de la lista de la LLM (quedan solo `calculateQuote` y `escalateToHuman`, funcionales); `kb.essentials()` inyecta SIEMPRE planes + qué cubre (independiente del match); `retrieve()` con fallback OR (`to_tsquery` con términos unidos por `|`) cuando el AND queda vacío; `runToolLoop` fuerza una respuesta en texto sin tools si agota rondas (nunca devuelve vacío). Verificado end-to-end contra neondb_test: nombra Plan A/B/C, cobertura/exclusiones/contacto/reclamación correctos, cotización real, tono humano, sin fugas de marcadores.
**Prevención:** en un bot de atención al cliente con LLM, preferir RAG-first (recuperar → inyectar → generar libre y anclado) sobre tool-heavy (obligar a la LLM a llamar tools para recuperar). Las tools son para acciones FUNCIONALES (cálculos deterministas, efectos), no para leer conocimiento que se puede inyectar en contexto. Cada tool en el toolset agrega tokens y superficie de fallo a CADA llamada. Y para RAG con full-text de Postgres: `websearch_to_tsquery`/`plainto_tsquery` son AND (bajo recall en consultas conversacionales) — tener un fallback OR y/o esenciales siempre-presentes.
**Archivos:** `src/brain/prompts/v2.system.ts`, `src/brain/prompt.manager.ts`, `src/composition.ts`, `src/domain/knowledge/rag.ts`, `src/brain/tools/registry.ts`, `src/shared/ports/index.ts`.
